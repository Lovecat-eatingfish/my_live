...
# NOTE: stream-provider references living + im-router (ImBroadcastServiceImpl),
# so it MUST start after them (Wave4c), not in Wave0.
$Waves = @(
  @{ Name="Wave0 base (no Dubbo dep)"; Mods=@("qiyu-live-bank-provider","qiyu-live-account-provider","qiyu-live-id-generate-provider","qiyu-live-im-provider") },
  ...
  @{ Name="Wave4c stream (after living+im-router)"; Mods=@("qiyu-live-stream-provider") },
...
# Dubbo 3.2 rejects 127.0.0.1 as DUBBO_IP_TO_REGISTRY and then the whole Dubbo module fails to
...
# but user<->living and living<->stream reference each other in code (dependency cycle).
# The weak edges below must be exempted by interface name, otherwise no start order
# can pass verification. Exemption affects startup check only; references still lazy-load.
$ModuleExtraArgs = @{
