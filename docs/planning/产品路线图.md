...
- **弹幕压测**：写 `scripts/e2e/im_benchmark.mjs`（Node WebSocket 并发客户端，参考现有 scripts/e2e/refresh_no_close_test.mjs 的写法），目标：拿到单机 im-core-server 在 1w/5w 连接下的广播 RT 基线，写入 docs。
