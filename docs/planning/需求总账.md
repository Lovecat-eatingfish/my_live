...
> 更新时间：2026-09-13。最初需求原文见 [archive/原始需求.md](../archive/原始需求.md)（已完成），历史迭代设计见 [archive/迭代归档-2026-09.md](../archive/迭代归档-2026-09.md)。
...
- **镜像**：`docker/app.Dockerfile` 通用 JRE17 镜像（`EXTRA_ARGS` 启动参数覆盖配置），`scripts/ops/build-images.sh` 一键构建 15 个服务镜像（`qiyu-live/<模块>:dev`）。
...
| 需求 | 实现 |
|------|------|
| 红包扣费（15） | 发送时 `consumeForRedPacket` 原子扣主播金币（余额不足 code 10111 拦截），流水类型 2=红包支出；发送后投递 1 分钟延迟 MQ 自动结算，未领完部分按类型 3=红包退还退回主播；顺带修复新用户无账户行导致入账丢失的问题（incr 改 upsert 自动建账）。E2E：`scripts/e2e/redpacket_deduct_test.mjs` 全过 |
...
curl 级 E2E：`scripts/e2e/medium_features_test.mjs` 9/9 全过；浏览器 E2E：改昵称生效、开播弹窗类型选择、房间顶栏观众数、商品管理上架/下架全部实测通过。
...
> 对照 [产品路线图.md](产品路线图.md) 逐项核对（2026-09-13，批次零~九完成后）。
...
新坑记录：踩坑记录.md §26（跨服务 Redis @class）、§27（延迟消息裸 body）、§28（grep 吞构建失败）。
