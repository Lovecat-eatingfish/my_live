...
| 六 | ✅ 已完成（2026-09-13） | 下一轮批次一 commit | 下一轮批次一（运营配置+点睛）：① 礼物/充值档位配置页走既有 RPC（IGiftConfigRpc 有完整 CRUD+缓存失效；IPayProductRpc 补 adminUpdateProduct 时踩缓存失效坑——products() 有 Redis List 缓存，改价必须同时清 list+item 两个 key）；② 点睛：进场欢迎（living userOnlineHandler 首次进房广播 system 弹幕，E2E 需轮询等 living→userRpc 冷连接）、深色模式（Element Plus dark css-vars + localStorage 记忆）、房间分享（复制链接）、主播看板（UserCenterPage tab：人气榜/在线数/贡献榜聚合）；③ E2E 7/7（其余见下） |
| 七 | ✅ 已完成（2026-09-13） | 下一轮批次二 commit | 连麦：① guest streamKey 前缀 liveg_（md5(roomId_guestId_secret)），走 Redis 反查 roomId 不占房间主 stream_key，on_publish 归属校验天然不误伤房间状态；② 状态机 t_living_linkmic invite(0)/accept(1)/hangUp(2)，5572 信令 invite 单发/start+stop 广播；③ 观众第二画面拉 guest 流 HLS（SRS 自动转），被邀请观众第二路 WebRTC 推流（复用主播采集逻辑）；④ 踩坑：living-provider 缺 lombok 依赖、b7 构建时 stream/living jar 因文件锁没更新成功导致 Service not found（老坑重演，进程必须先杀再 clean）；⑤ E2E linkmic_test.mjs 12/12（邀请/重复拒/接受/accepted 推流参数/广播/guest 流注册 SRS/挂断） |
| 八 | ✅ 已完成（2026-09-13） | 下一轮批次三 commit | 付费直播间：① t_living_room 加 pay_type/ticket_price，开播可选设票（StartLivingDialog 门票开关+价格）；② 简化决策：门票不走 mock 支付回调链路，直接金币扣款（decr 观众/incr 主播），Redis 购票标记 qiyu:live:ticket:{roomId}:{userId}（全局常量前缀，api 与 stream 共读，TTL 12h 兜底）；③ 鉴权点 anchorConfig（未购票→错误码 10114 前端弹购票窗）+ getPlayUrl（防蹭播）；④ 幂等靠购票标记 hasKey；⑤ 踩坑：连枚举插入时把上一行结尾逗号吃掉（编译错）、living-interface 字段没插入但 getter 先加（锚点行不存在），构造器用 QiyuErrorException(ApiErrorEnum) 直传枚举；⑥ E2E pay_ticket_test.mjs 8/8 |
| 九 | ✅ 已完成（2026-09-13） | 下一轮批次四 commit | PK竞技化+分账底座：① 分账：送礼扣费成功后主播入账 price×90%（平台抽成10%，SendGiftConsumer 内 incr），余额即分账底座（独立收益流水表留下一轮）；② 点赞加分：/living/pk/like 主播方+1（与送礼同 Lua 同 key clamp 0~100），每观众每日 50，广播 5558 复用进度通道；③ 倒计时结算：onlinePk 发 RocketMQ 延迟消息(10min)→PkSettleConsumer 判胜负广播 5573+置 isOver+清 key；提前打满路径 5558 带 winnerId；④ 修上游老 bug：onlinePk 成功路径 onlineStatus 误设 false（api 层把连线成功当错误抛 -1）；⑤ 本批最大坑：**RedisKeyBuilder 前缀取 spring.application.name，同一 builder 类跨服务产出不同前缀 key**——gift 写 qiyu-live-gift-provider:living_pk_key 而 living 读写成 qiyu-live-living-provider:living_pk_key，两套数据互不可见（PkConstants 固定前缀解决，is over 空格是历史 key 形状不能改）；⑥ E2E pk_share_test.mjs 7/7 |
