# 路线图实施计划（产品路线图.md 的执行版）

> 写作日期：2026-09-13
> 定位：`产品路线图.md`（同目录） 是产品路线图（不动它的内容），本文是**我的实现计划**——
...
1. **支付回调幂等**（`bank-provider/PayOrderServiceImpl.payNotify`）：✅ 已修
   ...
   - `payNotifyHandler` 改为条件更新 `SET status=2, pay_time=now WHERE order_id=? AND status IN (0,1)`，affected==0（并发双回调）不再入账/发 MQ。E2E：充值 1000 金币后重放同一回调 2 次，余额不变（scripts/e2e/paynotify_idempotent_test.mjs 全绿）。
2. **§6.12 清单逐条核实结论**：
...
> E2E：scripts/e2e/risk_control_test.mjs 13 项断言全过（昵称/标题/评论/房间名拦截、弹幕替换*、拦截+5566 单发、禁言、解禁恢复、频率 12→10、封号 403、解封恢复）。
> 过程中挖出并修复全项目级编码坑：JDK17 中文 Windows 默认 GBK 导致 MQ 中文消息乱码+解析失败全丢（详见 踩坑记录.md §19），start-all.sh 已加 -Dfile.encoding=UTF-8。
...
> E2E：scripts/e2e/relation_profile_level_test.mjs 15/15 全过（关注/取关/计数、双向列表、开播推送 5567+通知落库、主页聚合、TA的视频、看播+10/发视频+50/弹幕+1日限20、跨级升级+5570、弹幕 level 徽章）。
...
> E2E：scripts/e2e/rank_search_notify_test.mjs 15/15 全过（人气榜、本场贡献榜、主播日榜/周榜聚合、搜索三分栏、关注/点赞通知、未读数/单条已读/全部已读）。
...
> E2E：scripts/e2e/transcode_feed_test.mjs 12/12 全过（mpeg4/mov 重编码→h264+自动抽封面、H.264 mp4 remux+duration/size 修正、发布处理中详情不可见、feed 热度排序游标分页两页无交集、完播/未完播落库）。
...
> E2E：scripts/e2e/admin_audit_test.mjs 10/10 全过（仪表盘、审核队列→通过上线→驳回不可见、ffmpeg -re 真实推流→30s 截帧落库→警告/强关处置、敏感词增删查）；批次四 transcode_feed_test 回归 12/12 全过（审核步骤已并入）。
...
- **端口步进 5**：新 Dubbo/HTTP 端口按 `../ops/端口总表.md` 规则取号（下一步 Dubbo 从 30070 起，HTTP 从 38105 起若需新增），**改完必同步 端口总表.md**。
