...
> 配套文档：启动详见 [启动手册.md](启动手册.md)（依赖原理/顺序/验证/排错），停止用 `scripts/stop-all.bat`。
...
| 服务 | 地址 |
|---|---|
| C端 api | http://127.0.0.1:38085/live/api/swagger-ui/index.html |
| 运营台 admin-api | http://127.0.0.1:38100/live/admin/swagger-ui/index.html |
| 支付 bank-api | http://127.0.0.1:38095/live/bank/swagger-ui/index.html |

原始 OpenAPI JSON：`/v3/api-docs`（同样带各自 context-path）。admin-api 的 Swagger 路径已在 AdminAuthFilter 白名单放行。

## 敏感配置环境变量（application.yml 占位符）

| 变量 | 默认（本地开发） | 说明 |
|---|---|---|
| `QIYU_MYSQL_USERNAME` / `QIYU_MYSQL_PASSWORD` | root / 123456 | MySQL |
| `NACOS_USERNAME` / `NACOS_PASSWORD` | nacos / nacos | Nacos 控制台与注册中心 |
| `QIYU_MINIO_ACCESS_KEY` / `QIYU_MINIO_SECRET_KEY` | minioadmin / minioadmin | MinIO |
| `QIYU_SRS_SECRET` | your_secret_key_here | SRS 回调密钥 |
| `QIYU_SMS_MOCK_ENABLED` / `QIYU_SMS_MOCK_CODE` | true / 123456 | 短信测试模式；**上线必须置 false** |
