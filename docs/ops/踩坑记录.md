...
验证脚本：`scripts/e2e/stream_push_test.mjs`（推流链路）、`scripts/e2e/stream_kick_test.mjs`（关播踢流：推流保持中 closeLiving → 流必须销毁且 ffmpeg 被踢断）。
...
- 验证脚本 `scripts/e2e/stream_push_test.mjs` 全链路绿灯：登录 → 开播 → 签发推流地址 → FFmpeg 推流 → SRS 收流 → on_publish 回调 → FLV/HLS 播放 → 关播踢流清理 → DVR 落盘。
