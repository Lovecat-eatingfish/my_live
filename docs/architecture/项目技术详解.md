...
> 配套文档：`../ops/端口总表.md`（端口总表）、`数据库设计.md`、`流媒体架构.md`、`../ops/启动手册.md`、`../ops/踩坑记录.md`。
...
23. [部署与端口](#二十三部署与端口)
24. [付费直播间门票](#二十四付费直播间门票)
25. [PK 竞技化与分账底座](#二十五pk-竞技化与分账底座)
26. [私信 DM](#二十六私信-dm)
27. [直播分区与首页关注 Tab](#二十七直播分区与首页关注-tab)
28. [直播间口令抽奖](#二十八直播间口令抽奖)
29. [E2E 测试体系](#二十九e2e-测试体系)
30. [已知取舍与改进方向清单](#三十已知取舍与改进方向清单)
...
> 端口编号规则：Dubbo 端口 30010 起步长 5，详见 `../ops/端口总表.md`。
...
| 5558 | PK 送礼成功/点赞加分 | | 5565 | 直播间关闭 |
| 5559 | PK 连线 | | 5567 | 主播开播推送 |
| 5560/5561 | 红包雨发/领 | | 5568/5569 | 私信上行/下行 |
| 5570 | 用户升级特效 | | 5572 | 连麦信令 |
| 5573 | PK 倒计时结算广播 | | 5574/5575 | 口令抽奖发起/开奖 |
| 5566 | 风控拦截通知 | | | |
...
### 8.5 录制回放闭环（已打通，E2E 7/7）

完整链路：**主播推流 → SRS DVR 录制（FLV 格式，`dvr_plan session`，关播落盘）→ on_dvr 回调 → 上传 MinIO → 写回放表 → IM 广播 5564 → 主播主页展示**。

- **容器路径映射**：`toLocalPath` 把 SRS 回调里的容器路径 `/data/dvr/...` 映射为本机挂载路径 `./docker-data/srs/dvr/...`；本机原生 `srs.exe` 模式回调是 `./objs/dvr/...`，按部署模式在 application.yml 配 `dvr-container-prefix/local-dir`（踩坑：前缀匹配不上会导致文件被静默跳过）；
- **上传上下文**：on_publish 时 `captureStreamContext` 把 roomId/anchorId/streamKey/**recordEnabled**/startTime 写 Redis（TTL 2h）；未开启录制的房间关播后直接删本地文件；
- **异步上传**：2 线程 `record-upload-` 线程池执行——上传 MinIO（objectName `yyyy/MM/dd/{roomId}_{anchorId}_{endTime}.flv`）→ 写 `t_living_room_record`（room_id/anchor_id/record_url/duration 秒/file_size/start_time/end_time/**status 2=可用 3=失败**）→ 广播 5564 → 删本地文件；失败落 status=3；
- `t_living_room` 配套 `record_enabled` 字段，开播第 6 参透传，StartLivingDialog 加录制开关；
- **主页回放**：`ILivingPlayBackRpc.getRecordListByAnchor`（status=2 最新 20 条）→ api `POST /stream/recordsByAnchor` → 前端 ProfilePage 新增"直播回放"tab，复用 ReplayPlayer 组件；
- **修复记录**：PO `Date` → DTO `Long` 毫秒，`ConvertBeanUtils` 通用转换器不做类型转换，需显式回填 `.getTime()`（冒烟测试发现的回放时间显示为空 bug）。

> **注意**：SRS 4/5 的 DVR 由扩展名决定格式且无 `dvr_format` 指令，本项目录的是 **FLV**（无 ffmpeg 转 mp4 环节）；`qiyu-live-living.sql` 里的同名旧表是遗留错误版，以 `sql/streaming_module.sql` 为准。

---

## 九、连麦（linkmic）

> 已提交并打通 E2E（linkmic_test 12/12）：stream-provider 第二路推流 + living-provider 状态机 + 5572 信令 + 前端双画面。
...
- `t_living_room` 新增 `pay_type`(0 免费 / 1 门票) + `ticket_price`（金币），见 `sql/pay_living.sql`；
- 开播接口 `startingLiving` 新增 `payType/ticketPrice` 参数（另含 `recordEnabled`）：`payType==1` 时校验 `ticketPrice > 0`，否则抛"门票价格必须大于 0"。

### 24.2 购票流程（`LivingRoomServiceImpl.buyTicket`）

1. 非门票房直接返回 true；已持有票幂等返回；
2. `currencyAccountRpc.decr(观众, price)` 金币直扣 → `currencyAccountRpc.incr(主播, price)` 主播直接入账（无中间分账流水，记录在 `t_qiyu_currency_trade`）；
3. 写幂等标记：`qiyu:live:ticket:{roomId}:{userId} = "1"`，**TTL 12h**（`TicketConstants`，api 写、stream/api 读，全局固定前缀）。

> **取舍**：decr 与 incr 之间**无事务**，属"先扣后给"简单实现——incr 失败时观众已扣款。改进方向：本地消息表/事务消息保证两步最终一致。

### 24.3 进房双鉴权（防绕过）

- `anchorConfig`：非主播且 `payType==1` 时检查票 key，无票抛 `ApiErrorEnum.TICKET_REQUIRED`（枚举 code=14 × `ErrorAppIdEnum.QIYU_API_ERROR(101)` = **错误码 10114**），前端 RoomPage 弹购票窗；
- `getPlayUrl`：取播放地址前同样校验，防止绕过进房接口直接蹭流。

E2E：`scripts/e2e/pay_ticket_test.mjs` 8/8。
...
`SendGiftConsumer`（gift-provider）常量 `PROFIT_SHARE_RATIO = 0.9`：普通送礼扣款成功后 `anchorIncome = floor(price * 0.9)`，`currencyAccountRpc.incr(主播, anchorIncome)`——**观众扣全额，主播入账 90%，差额即平台收入（不产生独立分账流水表）**。入账失败仅记 error 不影响主流程（可改进：失败补偿/重试表）。

### 25.2 观众点赞加分 `/living/pk/like`

- 日限：key 含 `pklike:{userId}:{yyyyMMdd}`，INCR 后 expire 25h，**每人每日 50 次**；
- 加分：Lua `PK_LIKE_LUA`（不存在则初始化 50 再 incrby，钳制 0~100），与送礼进度**同 key 同 Lua 语义**：`qiyu-live-gift-provider:living_pk_key:{roomId}`；
- 广播 5558（复用 PK 送礼通道，data 带 `pkNum/likeAdd:true`）。

### 25.3 倒计时自动结算（延迟消息）

```mermaid
flowchart LR
    A[onlinePk 连线成功] -->|RocketMQ 延迟消息<br/>LivingPkSettleTopic<br/>delayTimeLevel=14 即10min| B[PkSettleConsumer]
    B --> C{pkNum key存在?}
    C -->|不存在=送礼已提前打满| D[跳过并清key]
    C -->|存在| E{判胜负}
    E -->|>50| F[主播胜]
    E -->|<50| G[连麦方胜]
    E -->|=50| H[平局 winnerId=null]
    F & G & H --> I[广播5573给房间在线用户集合]
    I --> J[置isOverKey TTL2h<br/>删pkNumKey/onlinePkKey]
```

### 25.4 两个重要修复（面试可讲的踩坑）

1. **老 bug**：`onlinePk` 成功分支误写 `respDTO.setOnlineStatus(false)`，导致 api 层 `ErrorAssert.isTure(...)` 把"连线成功"当失败抛出——改为 `true` 并补发延迟结算消息；
2. **RedisKeyBuilder 跨服务前缀陷阱**：`getPrefix()` 取各应用自己的 `spring.application.name`——在 living-provider 里注入 `GiftProviderCacheKeyBuilder` 产出的 key 前缀会变成 living 的应用名，导致 **gift 写 / living 读的 PK 进度"数据分裂"**。修复：跨服务共享 key 一律用 `PkConstants` 固定前缀（注意 `living_pk_is over:` 中间的**空格是历史 key 形状不能改**）。

> **学习点**：第 2 条是"封装的便利性 vs 隐式契约"的经典案例——KeyBuilder 按应用名做前缀在单服务内是优点，跨服务共享时就是陷阱；修复方式是把跨服务 key 升级为"全局常量显式写死前缀"。

E2E：`scripts/e2e/pk_share_test.mjs` 7/7。

---

## 二十六、私信 DM

### 26.1 链路（bizCode 5568 上行 / 5569 下行）

1. 前端**独立 IM 连接**（roomId=0，仅借登录包完成 uid→ip 绑定，`MessagePage.vue`）发 5568；
2. msg-provider `SingleMessageHandlerImpl#handleDmUp`：**fromUid 以连接鉴权结果为准防伪造** → 敏感词风控（SCENE_DANMU，拦截/替换）→ 落库 `t_user_dm_message` → **会话双方各一行原子 upsert**（`UserDmConversationMapper#upsertOnNewMsg`：发送方 unread+0、接收方 unread+1）；
3. 5569 **同时发给接收方与发送方回显**——前端"只以服务端回显为准追加"，天然有序、多端一致；
4. 接收方离线时 5569 自然丢弃，靠**未读数兜底**。

### 26.2 表设计（qiyu-live-msg 库，`sql/dm.sql`）

| 表 | 关键点 |
|---|---|
| `t_user_dm_message` | from_uid/to_uid/content(500)/status(1正常 0撤回)；`idx_from_to_id` + `idx_to_from_id` **双向索引**（查历史两个方向都走索引） |
| `t_user_dm_conversation` | owner_uid/peer_uid/last_msg/unread_cnt/update_time；`uk_owner_peer` 唯一键（每人每会话一行）；`idx_owner_update`（会话列表按 update_time 倒序 limit 100） |

### 26.3 接口与前端

- `IDmRpc`（common-interface）→ `DmRpcImpl`：会话列表 / 历史（默认 50 条最多 200）/ markRead / unreadTotal；
- api `DmController`：`GET /dm/conversations`（peer 昵称头像经 `IUserRpc.batchQueryUserInfo` 富化）、`GET /dm/history`、`POST /dm/markRead`；
- 前端 `MessagePage.vue`（会话列表+聊天窗）；首页导航"消息" tab + 未读角标 60s 轮询；
- **冒烟修复**：直链访问时 userStore 未初始化导致 IM 登录失败，onMounted 补 `fetchUserInfo()`。

E2E：`scripts/e2e/dm_test.mjs` 12/12。

---

## 二十七、直播分区与首页关注 Tab

- **分区表** `t_living_category`（qiyu_live_living）：id 自增（即房间 type 值）/ name 唯一键 / icon / sort（越小越靠前）/ status(1 启用 0 停用)；种子数据：娱乐🎭 / 游戏🎮 / 赛事🏆 / 带货🛒；
- admin 侧 `LivingCategoryPage` 管理页（增改/启停/排序）；C 端 `POST /living/categories` RPC 全量后过滤 status==1，前端加载失败兜底内置四分区；
- **关注 tab**：`POST /living/followRooms` → 先 `followList(page,200)` 拿关注主播 id 列表 → `livingRoomRpc.listByAnchorIds`：`status=1(开播中) AND anchor_id IN (...) ORDER BY id DESC` 分页（房间 id 即开播时间序）；`hasNext = 房间页 hasNext && 关注页 hasNext`；
- 前端 HomePage 分区 tab 动态化，登录用户渲染"➕ 关注"tag，`currentType==='follow'` 时走 followRooms 否则走 listRoom。

> **取舍**：关注列表先拉后查（两次查询）而非 SQL join——user 库与 living 库**分库**，跨库 join 不可行；关注数上限 200 控制了 IN 子句规模。

E2E：`scripts/e2e/category_follow_test.mjs` 12/12。

---

## 二十八、直播间口令抽奖

**无新表，状态全在 Redis**（跨服务 living 写 / msg 读，用固定前缀 + StringRedisTemplate——见 §26 的序列化坑）。

### 28.1 Redis 结构（`LotteryConstants`）

| Key | 结构 | 说明 |
|---|---|---|
| `...:living_lottery:{roomId}` | String JSON | 上下文：roomId/keyword/winnerCount/rewardCoins/anchorId/endTime，TTL=时长+120s |
| `...:living_lottery_participants:{roomId}` | SET | SADD 去重，成员=userId |

### 28.2 流程（bizCode 5574 发起 / 5575 开奖）

1. **发起** `POST /living/lottery/create`：仅主播、时长必须在映射表内（30s→延迟级别4、60s→5、120s→6、180s→7、300s→9）、winnerCount 1~100、rewardCoins≥0、无进行中抽奖；`rewardCoins>0` 先校验余额再 `decr` 主播扣奖池 → 写 ctx + 清参与者集合 → 广播 5574（观众可见口令）→ 发延迟消息 `LivingLotterySettleTopic`（body 裸 roomId）；
2. **参与**：msg-provider 弹幕处理链里 `checkLotteryJoin`——房间 ctx 存在且 `content.trim() == keyword` **全字匹配** → SADD，静默无提示；
3. **开奖** `LotterySettleConsumer`：ctx 过期直接返回 → **`Collections.shuffle` 后取前 N 人** → 奖励均分 `rewardCoins / actualWinners`（整数除，**余数退回主播**），无人参与全额退回 → 广播 5575 给参与者+主播 → 删 ctx/集合。

> **学习点**：延迟消息用 RocketMQ 固定档位（delayTimeLevel）而非精确延时——档位映射表就是为绕开这个限制设计的；裸 roomId 作 body 引出过"消息体必须可序列化且带类型信息"的坑（troubleshooting §27）。

E2E：`scripts/e2e/lottery_test.mjs` 13/13。

---

## 二十九、E2E 测试体系

`scripts/` 下 **20+ 个独立 `.mjs` 脚本**（Node 原生 HTTP/WebSocket 直连，无测试框架），每个功能批次一个脚本：`e2e_test`（主链路）、`pay_ticket`、`pk_share`、`dm`、`category_follow`、`record_replay`、`lottery`、`linkmic`、`ops_config`、`admin_audit`、`cart`、`medium_features`、`rank_search_notify`、`recharge`、`paynotify_idempotent`、`redpacket_deduct/pool`、`refresh_no_close`、`relation_profile_level`、`risk_control`、`stream_kick`、`stream_push`、`transcode_feed` + 双观众辅助 `viewer_b`。

- 组织方式：脚本内自建用户/登录/（必要时 ffmpeg 真实推流），逐断言打印 ✅/❌，**退出码收尾**；提交信息里的 "12/12" 即断言计数；
- **stream_kick 判定窗口 10s→20s**：SRS 关播踢流有延迟，10s 窗口下 ffmpeg 可能尚未退出被误判 still-running 造成偶发误报；
- 涉及真实流媒体的脚本（record_replay / stream_kick / stream_push）会真实 ffmpeg 推流到 SRS 验证完整回调链路。

> **学习点**：无框架的原生脚本 E2E——轻量、零依赖、随 batch 提交，是个人项目里性价比最高的质量保障方式；配套 `../planning/复查清单.md` 复查清单和 troubleshooting（27+ 条踩坑记录）。
...
| 11 | 门票购票 decr 与主播 incr 之间无事务 | 本地消息表 / RocketMQ 事务消息保证最终一致 |
| 12 | PK 分账 90/10 无独立分账流水表，主播入账失败仅记日志 | 建分账流水表 + 失败补偿重试 |
| 13 | 回放为 FLV 格式（SRS DVR 限制） | on_dvr 后加 ffmpeg 转 mp4(faststart) 支持拖动进度条 |
| 14 | 抽奖/门票等跨服务 Redis key 部分靠固定常量前缀约定 | 统一 KeyBuilder 支持显式指定前缀，规范 key 治理 |
