  ...
  <div class="home-page">
    ...
    <div class="room-grid" v-else>
      <div v-for="room in rooms" :key="room.id" class="room-card" @click="$router.push(`/room/${room.id}`)">
        <div class="cover-wrap" :class="{ 'has-pay': room.payType === 1 }">
          <img :src="room.covertImg || defaultCover" class="cover" loading="lazy" />
          <span v-if="room.payType === 1" class="pay-badge">付费</span>
...
.watch-num { right: 6px; }
.good-num { left: 6px; }
/* 付费直播间：右下角红色角标；有人数角标时把它往左让位 */
.pay-badge { position: absolute; right: 6px; bottom: 6px; font-size: 12px; font-weight: 600; color: #fff; background: #e53935; padding: 2px 7px; border-radius: 4px; }
.has-pay .watch-num { right: 52px; }
